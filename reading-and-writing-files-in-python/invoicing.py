...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    parsed_invoice_number = invoice_number.split('-')
    year, number = parsed_invoice_number
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    parsed_invoice_number = parse_invoice_number(invoice_number)
    if parsed_invoice_number[0] == current_year:
        return f"{parsed_invoice_number[0]}-"+f"{parsed_invoice_number[1]+1:04d}"
    else:
...
def record_invoice(invoice_file: TextIO,
    ...
    :param last_line_ptr: the position denoting the beginning of the last line in the file. This will be obtained by the
    previous call to `record invoice`.
    :return: the position of the beginning of the last line. This can be used in subsequent calls to `record_invoice`.
    """
    ...
    if last_row:
        ...
        # if file empty, start numbering from one
        year = get_year()
...
data_file = 'Resources/invoices.csv'
